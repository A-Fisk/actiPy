    ...
    **kwarg = period_list
    ...
    def split_period(self):
        ...
        for PIRno, period in zip(df.columns, Period_list):
            ...
            # select each day in turn, can we put into a list and then concat
            # together later?
...
class Circadian_Analysis(split_by_period):
    ...
    def Enright_Periodogram(self, low, high):
        ...
        for test_period in test_periods:
            ...
            for PIRno, split_list in zip(
                    data.columns, split_by_test_period.split_completed_list):
    ...
    def LombScargle_period(self, low=20, high=30):
        ...
        for PIR in a.columns:
            ...
            power_temp = LombScargle(
                observation_times, observations).power(
                freq, method='auto')
            power_list.append(power_temp)

            # check if has actually produced something, if not put everything
            # as 0
            if pd.isnull(power_temp[0]):
                pmax_list.append(0)
        ...
        # put these into DataFrame to make more useable. One for period and
        # pmax values
        ...
        self.LS_Period_list = [(str(int(item)) +
                                "H " +
                                str(int(round((item -
                                               int(item)) *
                                    60))) +
                                "T") for item in Period_list]
...
class Mean_Waveforms(Circadian_Analysis, split_by_period):
    ...
    **kwargs
    ...
    def waveform_define(self,
        ...
        # loop through all the different light periods and select those
        # portions separately
        ...
        for temp_class in itertools.chain(
                split_24_class_list, split_LS_class_list):
